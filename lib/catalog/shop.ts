...
export const getShopCatalog = cache(async ({
  category,
  subcategory,
  query,
  page = 1,
  ...
  if (activeCategory && activeSubcategory && search) {
    const [products, totalRows, subcategoryRows, categoryRows] = await Promise.all([
      sql.query(
...
export const getFeaturedShopProducts = cache(async (): Promise<ShopProduct[]> => {
  const sql = getSql();
